function createUnityInstance(e, t, r) {
    ...
    function o(e) {
        ...
        if (n.startsWith(r) && (n = n.substring(r.length)), r += "\n" + n.trim(), r && f.stackTraceRegExp && f.stackTraceRegExp.test(r)) {
            ...
            try {
                for (var o in v)
                    if (v[o].hasUnityMarker(e)) {
                        ...
                        var a = v[o];
                        if (!a.worker) {
                            var s = URL.createObjectURL(new Blob(["this.require = ", a.require.toString(), "; this.decompress = ", a.decompress.toString(), "; this.onmessage = ", function (e) {
                                var t = {
                                    id: e.data.id,
                            ...
                            a.worker = new Worker(s), a.worker.onmessage = function (e) {
                                return e.data.ready ? void URL.revokeObjectURL(s) : (this.callbacks[e.data.id](e.data.decompressed), void delete this.callbacks[e.data.id])
                            }, a.worker.callbacks = {}, a.worker.nextCallbackId = 0
                        }
                        var i = a.worker.nextCallbackId++;
                        return a.worker.callbacks[i] = r, void a.worker.postMessage({
                            id: i,
                            compressed: e
                        }, [e.buffer])
                    } r(e)
            ...
            return new Promise(function (e, r) {
                ...
                o.src = t, o.onload = function () {
                    if ("undefined" == typeof unityFramework || !unityFramework) {
                        ...
                        for (var a in r) {
                            var s = r[a];
                            if (f.frameworkUrl.endsWith("." + s[0])) {
                                var i = "Unable to parse " + f.frameworkUrl + "!";
                                if ("file:" == location.protocol) return void n(i + " Loading pre-compressed (brotli or gzip) content via a file:// URL without a web server is not supported by this browser. Please use a local development web server to host compressed Unity content, or use the Unity Build and Run option.", "error");
                                if (i += ' This can happen if build compression was enabled but web server hosting the content was misconfigured to not serve the file with HTTP Response Header "Content-Encoding: ' + s[1] + '" present. Check browser Console and Devtools Network tab to debug.', "br" == s[0] && "http:" == location.protocol) {
                                    var d = ["localhost", "127.0.0.1"].indexOf(location.hostname) != -1 ? "" : "Migrate your server to use HTTPS.";
                                    i = /Firefox/.test(navigator.userAgent) ? "Unable to parse " + f.frameworkUrl + '!<br>If using custom web server, verify that web server is sending .br files with HTTP Response Header "Content-Encoding: br". Brotli compression may not be supported in Firefox over HTTP connections. ' + d + ' See <a href="https://bugzilla.mozilla.org/show_bug.cgi?id=1670675">https://bugzilla.mozilla.org/show_bug.cgi?id=1670675</a> for more information.' : "Unable to parse " + f.frameworkUrl + '!<br>If using custom web server, verify that web server is sending .br files with HTTP Response Header "Content-Encoding: br". Brotli compression may not be supported over HTTP connections. Migrate your server to use HTTPS.'
                                }
                                return void n(i, "error")
                            }
                        }
    ...
    function c() {
        Promise.all([l(), u("codeUrl")]).then(function (e) {
        ...
        f.preRun.push(function () {
            f.addRunDependency("dataUrl"), e.then(function (e) {
                ...
                for (r += 4; r < o;) {
                    ...
                    r += i;
                    for (var u = 0, l = d.indexOf("/", u) + 1; l > 0; u = l, l = d.indexOf("/", u) + 1) f.FS_createPath(d.substring(0, u), d.substring(u, l - 1), !0, !0);
    ...
    }), window.addEventListener("error", o), window.addEventListener("unhandledrejection", o), f.deinitializers.push(function () {
        f.disableAccessToMediaDevices(), p.forEach(function (t) {
    ...
    document.addEventListener("webkitfullscreenchange", function (t) {
        ...
        r === e ? e.style.width && (g = e.style.width, m = e.style.height, e.style.width = "100%", e.style.height = "100%") : g && (e.style.width = g, e.style.height = m, g = "", m = "")
    });
    var b = {
        Module: f,
    ...
    f.SystemInfo = function () {
        ...
        for (var t, r, n, o, a, s, i = navigator.userAgent + " ", d = [
                ["Firefox", "Firefox"],
                ["OPR", "Opera"],
                ["Edg", "Edge"],
        ...
        return s(e, "", 0), !0
    }, Error.stackTraceLimit = Math.max(Error.stackTraceLimit || 0, 50), f.fetchWithProgress = function () {
        function e(e, t) {
            if (!t) return 0;
            ...
            return r && r.onProgress && (n = r.onProgress), fetch(t, r).then(function (t) {
                function r() {
                    return "undefined" == typeof a ? t.arrayBuffer().then(function (e) {
                        ...
                        return e.done ? o() : (l + e.value.length <= d.length ? (d.set(e.value, l), c = l + e.value.length) : u.push(e.value), l += e.value.length, n({
                            type: "progress",
                            total: Math.max(i, l),
                            loaded: l,
                            lengthComputable: s
                ...
                var a = "undefined" != typeof t.body ? t.body.getReader() : void 0,
                    ...
                    l = 0,
                    c = 0;
                return s || console.warn("[UnityCache] Response is served without Content-Length header. Please reconfigure server to include valid Content-Length for better download performance."), r().then(function (e) {
                    return n({
                        type: "load",
                        total: e.length,
                        loaded: e.length,
        ...
        function s(s, l) {
            function c(t, r) {
                return u(t, r).then(function (t) {
                    return !g.enabled || g.revalidated ? t : 304 === t.status ? (g.result.revalidated = g.result.accessed, g.revalidated = !0, h.storeRequest(g.result).then(function () {
                    ...
                    }), new n(g.result.response)) : (200 == t.status ? (g.result = o(t.url, g.company, g.product, g.accessed, t), g.revalidated = !0, h.storeRequest(g.result).then(function () {
                    ...
                    })) : e("'" + g.result.url + "' request failed with status: " + t.status + " " + t.statusText), t)
                })
            }
            ...
            return l && (g.control = l.control, g.company = l.company, g.product = l.product), g.result = o(p, g.company, g.product, Date.now()), g.revalidated = !1, g.enabled ? h.loadRequest(g.result.url).then(function (t) {
                ...
                if (r(g.result.url) && (o.headers.get("Last-Modified") || o.headers.get("ETag"))) return fetch(g.result.url, {
                    method: "HEAD"
                }).then(function (t) {
                    return g.revalidated = ["Last-Modified", "ETag"].every(function (e) {
                        return !o.headers.get(e) || o.headers.get(e) == t.headers.get(e)
                    }), g.revalidated ? (g.result.revalidated = g.result.accessed, h.storeRequest(g.result), e("'" + g.result.url + "' successfully revalidated and served from the indexedDB cache"), f(o), o) : c(s, l)
                });
                l = l || {};
                var a = l.headers || {};
                return l.headers = a, o.headers.get("Last-Modified") ? (a["If-Modified-Since"] = o.headers.get("Last-Modified"), a["Cache-Control"] = "no-cache") : o.headers.get("ETag") && (a["If-None-Match"] = o.headers.get("ETag"), a["Cache-Control"] = "no-cache"), c(s, l)
            }).catch(function (t) {
